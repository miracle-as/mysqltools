...
my $sth      = $dbh->prepare("select table_schema, table_name, engine from information_schema.tables where engine like 'myisam' and table_schema not in ( 'mysql', 'information_schema' ) ");
$sth->execute();
